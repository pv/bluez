...
#define REREAD_LONG_CHANGE_START \
	NOTIFY_CHRC(TRACK_CHG), \
	READ_CHRC(TRACK_TITLE, BLOB_DATA_MTU_1('y')), \
	/* Changed during read -> reread */ \
...
#define REREAD_LONG_CHANGE \
	REREAD_LONG_CHANGE_START, \
	/* Reread attempt */ \
	READ_CHRC(TRACK_TITLE, BLOB_DATA_MTU_1('x')), \
	/* Changed during read again -> reread yet again */ \
	READ_BLOB_ERR_CHRC(TRACK_TITLE, 0x40 - 1, 0x80), \
	READ_CHRC(TRACK_TITLE, BLOB_DATA_MTU_1('x')), \
	READ_BLOB_CHRC(TRACK_TITLE, 0x40 - 1, 'x', 'x', 'x'), \
	IOV_NULL

#define CL_BLUEZ_2_REREAD	REREAD_LONG_CHANGE
...
#define CL_BLUEZ_4_REREAD \
	REREAD_LONG_CHANGE_START, \
	READ_CHRC(TRACK_TITLE, BLOB_DATA_MTU_1('y')), \
	/* Give up after too many reread attempts */ \
	READ_BLOB_ERR_CHRC(TRACK_TITLE, 0x40 - 1, 0x80), \
	READ_CHRC(TRACK_TITLE, BLOB_DATA_MTU_1('y')), \
	READ_BLOB_ERR_CHRC(TRACK_TITLE, 0x40 - 1, 0x80), \
	READ_CHRC(TRACK_TITLE, BLOB_DATA_MTU_1('y')), \
...
static void cl_reread_long_fail_track_title(void *user_data,
					const uint8_t *value, uint16_t length)
{
	if (strncmp((void *)value, "Title", length))
		FAIL_TEST();
}

static void cl_reread_long_fail_track_changed(void *user_data)
{
	struct test_data *data = user_data;
...
const struct test_config cfg_cl_bluez_4_reread = {
	.listener_cb = &(struct bt_mcp_listener_callback) {
		.track_title = cl_reread_long_fail_track_title,
		.track_changed = cl_reread_long_fail_track_changed,
	},
	.setup_data = setup_data_mcs,
	.setup_data_len = ARRAY_SIZE(setup_data_mcs),
	.gmcs = false,
...
static void testgroup_cl_extra(void)
{
	...
	define_test("MCP/CL/BLUEZ-2 [Long Value Reread, No Notify]",
		test_setup, test_client,
	...
	define_test("MCP/CL/BLUEZ-4 [Long Value Reread, Failure]",
		test_setup, test_client,
		&cfg_cl_bluez_4_reread, CL_BLUEZ_4_REREAD);
}
