			...
			void *cb_data, void *user_data)
{
	...
	if (!media_endpoint_async_call(msg, endpoint, NULL, pac_select_qos_cb,
								data, free))
		return -EIO;
...
static void pac_cancel(struct bt_bap_pac *lpac, void *cb, void *cb_data,
								void *user_data)
{
	struct media_endpoint *endpoint = user_data;
	...
	while (l) {
		...
		if (req->cb == pac_select_cb) {
			struct pac_select_data *data = req->user_data;
		...
		} else if (req->cb == pac_select_cb) {
			struct pac_select_qos_data *data = req->user_data;

			if (bt_bap_stream_get_lpac(data->stream) != lpac ||
						data->cb != cb ||
						data->user_data != cb_data) {
				l = g_slist_next(l);
				continue;
