...
static void send_iso(struct btdev_conn *conn, uint8_t pb, bool ts,
		uint16_t sn, uint32_t timestamp, uint16_t slen,
		const void *data, uint16_t len)
{
	...
	util_debug(conn->dev->debug_callback, conn->dev->debug_data,
				"send iso ts %x pb %x time %u sn %x slen %x",
				ts, pb, timestamp, sn, slen);

	memset(out, 0, sizeof(out));

	out[0].iov_base = &pkt_type;
	out[0].iov_len = sizeof(pkt_type);

	hdr.handle = acl_handle_pack(conn->handle, iso_flags_pack(pb, ts));
	out[1].iov_base = &hdr;
	out[1].iov_len = sizeof(hdr);

	if (ts) {
		timestamp = cpu_to_le32(timestamp);
		out[2].iov_base = &timestamp;
		out[2].iov_len = sizeof(timestamp);
	}
	...
	if (!(pb & 0x1)) {
		...
		out[3].iov_base = &data_hdr;
		out[3].iov_len = sizeof(data_hdr);
	}

	out[4].iov_base = (void *) data;
	out[4].iov_len = len;

	for (i = 2; i < ARRAY_SIZE(out); ++i)
		dlen += out[i].iov_len;

	hdr.dlen = cpu_to_le16(dlen);

	send_packet(conn->link->dev, out, ARRAY_SIZE(out));
}
...
static void process_iso(struct btdev *dev, const void *data, uint16_t len)
{
	...
	send_iso(conn, pb, ts, sn, timestamp, slen, in.iov_base, in.iov_len);
}
