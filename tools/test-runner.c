...
static bool check_virtiofsd(void)
{
	...
	/* virtiofsd wants some capabilities when run as root, or it refuses to
	 * start. They are not required when not run as root.
	 */
	if (geteuid() != 0)
		return true;

	if (syscall(SYS_capget, &caphdr, &capdata)) {
		perror("syscall");
		return false;
	}

	caps = capdata[1].effective;
	caps <<= 32;
	caps |= capdata[0].effective;

#define CHECK_CAPABILITY(cap) \
		if (!(caps & (1ULL << (cap)))) { \
			fprintf(stderr, "Run as root, but " #cap " missing"); \
			return false; \
		}

	CHECK_CAPABILITY(CAP_CHOWN)
	CHECK_CAPABILITY(CAP_DAC_OVERRIDE)
	CHECK_CAPABILITY(CAP_FOWNER)
	CHECK_CAPABILITY(CAP_FSETID)
	CHECK_CAPABILITY(CAP_SETGID)
	CHECK_CAPABILITY(CAP_SETUID)
	CHECK_CAPABILITY(CAP_MKNOD)
	CHECK_CAPABILITY(CAP_SETFCAP)
...
static void usage(void)
{
	...
	printf("Options:\n"
		...
		"\t-V, --virtiofs[=path/no]  Use or disable virtio-fs\n"
...
static const struct option main_options[] = {
	...
	{ "virtiofs", optional_argument, NULL, 'V' },
...
int main(int argc, char *argv[])
{
	...
	for (;;) {
		...
		opt = getopt_long(argc, argv, "au::bdsl::mq:Hk:A::U:P:o:V::vh",
						main_options, NULL);
		if (opt < 0)
			break;
